...
We benchmark a wide range of lightweight and efficient architectures on the AvocadoLeaf test set. Table~\ref{tab:arch_compare} reports top-1 accuracy, macro F1 score, number of parameters, and per-image inference time on a single NVIDIA GTX 3050-6GB GPU for all 15 evaluated models. All models are trained with the same data augmentation and optimisation schedule.
...
\caption{Macro F1 scores of all evaluated models. Lightweight models (<5M parameters) are shown in blue, heavy models (≥5M) in orange.}
...
% Pruning reduces the model size significantly (≈30\%) while the accuracy drop remains below 1\% for both models. This demonstrates that our architecture can be further compressed for resource‑constrained edge devices without major performance loss.

% \subsubsection{Error Analysis}
% We examine the confusion matrix on the test set (Figure~\ref{fig:confusion}) to understand the failure modes of ConvNeXt‑V1.
% % (Insert confusion matrix plot here, reference as \ref{fig:confusion})
% The most frequent confusions occur between visually similar classes, such as [Class A] vs [Class B] (e.g., “Speed limit 60” and “Speed limit 80”), accounting for 12\% of total errors. Some images are heavily occluded or suffer from motion blur, which suggests that additional data augmentation (e.g., random occlusion, motion blur) could improve robustness. Qualitative examples of correct predictions and errors are provided in Appendix~\ref{app:error_examples}.


% \subsubsection{Model Pruning \& Distillation}
...
% \midrule
% ConvNeXt‑V1 (Ours) & 5.2 & 3.6 & 0.82 \\
% MobileNet‑V2       & 4.3 & 3.0 & 0.55 \\
% \bottomrule
% \end{tabular}
% \end{table}
