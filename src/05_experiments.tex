...
To jointly assess accuracy, model size, and inference speed, we define a study-specific composite efficiency score:
...
Per-class recall ranges from 96.9\% (\textit{domla}, the largest class at 458 training images) down to 83.3\% (\textit{moctrang}, 136 training images). The three smaller classes, \textit{repsap} (115 training images, the smallest, 84.0\% recall), \textit{gisat} (128, 88.9\%), and \textit{moctrang}, all fall short of the 91\%+ recall reached by the three larger classes (\textit{domla} 458, \textit{binhthuong} 245, \textit{chaymepla} 220), broadly consistent with a sample-size effect. \textit{Moctrang} is nonetheless a partial exception to that pattern: despite having more training images than both \textit{repsap} and \textit{gisat}, it has the lowest recall of the six classes, suggesting that beyond sample size, its diffuse white-powdery symptom pattern (frequently confused with \textit{chaymepla}, discussed below) is intrinsically harder to classify. The confusions are concentrated in three symmetric pairs: \textit{gisat} (algal leaf spot, orange velvety pustules) $\leftrightarrow$ \textit{moctrang} (powdery mildew, white powdery coating) account for 5 of 22 misclassified test images in both directions, \textit{repsap} (mealybug damage, silver/stippled marks) is confused with \textit{chaymepla} (leaf scorch, margin/tip burn) in 12.0\% of \textit{repsap} images, and \textit{moctrang} is likewise confused with \textit{chaymepla} in 10.0\% of its images. All three pairs share a plausible visual explanation: each involves a diffuse or patchy discolouration (algal spots, powdery coating, insect stippling) that can visually resemble scorch damage or a different spot pattern under variable field lighting, rather than the compact, high-contrast lesions of \textit{domla}. Notably, \textit{repsap} is also the class with the lowest LIME faithfulness score (Section~\ref{subsec:interpretability}, mean confidence drop 0.416 vs.\ 0.649 overall), so its diffuse, non-contiguous damage pattern appears to make it both harder to classify correctly and harder to explain with a fixed-size superpixel mask, two independent pieces of evidence pointing at the same underlying cause.

\subsection{Model Interpretability}
\label{subsec:interpretability}

To assess whether ConvNeXt‑V2 relies on disease‑relevant visual evidence rather than spurious correlations, we apply LIME \citep{ribeiro2016why} to every image in the 280-image test set (not merely a qualitative sample). Each image is segmented into superpixels with the Quickshift algorithm, and 500 perturbed samples are used to fit a local surrogate model that identifies the superpixels most responsible for the predicted class.

We quantify faithfulness with a deletion metric. For each test image, we hide exactly the superpixels LIME identifies as most responsible for the predicted class (the same masking colour used during LIME's own perturbation sampling) and re-run the model, measuring how much its confidence in the predicted class drops. A faithful explanation should cause a large drop, since it claims the hidden region is what the prediction relies on. Averaged over all 280 test images, hiding the LIME-highlighted region (which covers only 20.0\% of the image area on average) drops the model's confidence in its own predicted class from 0.940 to 0.292, a mean confidence drop of 0.649 (bootstrap 95\% CI [0.618, 0.678], see \ref{app:lime_faithfulness} for the full methodology and per-class breakdown),demonstrating that predictions rely strongly on the LIME-attributed regions. The drop is larger for correctly classified images (0.660, $n=258$) than for misclassified ones (0.516, $n=22$), qualitatively, these highlighted regions visually align with disease-associated symptoms rather than background noise. This full-test-set quantitative result substantially strengthens the qualitative examples below.
...
Across the full 140-image escalated subset, the VLM judged the visual evidence to support the CNN's diagnosis for 92.0\% of the 125 images that produced a parseable response. This figure should not be read as a validated accuracy or quality score. It is simply how often the model answered ``yes'', and it is in fact somewhat higher than ConvNeXt‑V2's own accuracy on either grouping, 84.8\% on the same 125-image parseable subset or 85.0\% on the full 140-image escalated set (\ref{app:llm}). This is consistent with the low recall (0.21) for detecting actual mistakes reported in Section~\ref{sec:discussion}, as the model agrees with the CNN more often than the CNN is actually correct, suggesting a degree of acquiescence bias rather than critical evaluation of the evidence.
...
For each setting, we randomly sampled \(K\) images per class (\(K \in \{1,5,10,20\}\)) from the training set to form the support set, and evaluated on the full test set. Fine‑tune was trained for 50 epochs with early stopping. For ProtoNet and RelationNet we report the mean accuracy and standard deviation over 600 randomly generated episodes (6‑way, \(K\)-shot, 5 query images per class), drawn without a fixed random seed, so a repeated run reproduces the qualitative ranking below but not the exact reported figures within roughly one standard deviation.
...
Overall, the ProtoNet framework combined with a frozen ConvNeXt‑V2 backbone provides the strongest few‑shot baseline among the three methods evaluated, more than doubling fine‑tuning's accuracy at every directly comparable shot level (5, 10, and 20 shots) and reaching 57.4\% accuracy with only 20 labelled images per class. While this is well below the $>$90\% accuracy achieved with the full training set (Table~\ref{tab:arch_compare}), it demonstrates that metric-based few-shot learning is a substantially more data-efficient starting point than fine-tuning for scenarios where data collection is expensive.
...
Section~\ref{subsec:cross_dataset}.1 characterises behaviour on off-task inputs, but does not answer the question that matters most for external validity: trained only on AvocadoLeaf (domain A, collected first-hand in Vietnam), does BoVien still recognise avocado leaf disease on leaves it has never seen, collected independently, in a different country, under a different camera and background? K-Kotagiri \citeyearpar{kotagiri2024avocado} (Table~\ref{tab:dataset_comparison}), a 435-image, healthy/diseased-labelled avocado leaf dataset collected in Tamil Nadu, India, lets us test this directly for the first time. Because Kotagiri provides only binary labels, BoVien's 6-way prediction is collapsed to binary (\textit{binhthuong} $\to$ healthy, the other five disease classes $\to$ diseased) for this comparison, a same-task but coarser test than the full 6-class problem. To isolate genuine domain shift in leaf and disease appearance from a simple background-format mismatch, every Kotagiri image is passed through the same background-removal preprocessing (\texttt{rembg}-based foreground segmentation, crop, resize to $224\times224$) used to build the processed AvocadoLeaf training data itself, rather than fed to the model on its original concrete/stone background.
...
The architecture comparison in Section~\ref{subsec:arch_compare} reports GPU inference latency measured on batched, pre-processed test images, which does not speak to true embedded/mobile hardware, the setting edge deployment claims are ultimately about. To close this gap, we exported BoVien to ONNX and deployed it on four platforms. On Windows, ONNX Runtime runs the model with its CPU execution provider. On macOS (iMac), ONNX Runtime is configured to use its Core ML execution provider when available, falling back to CPU otherwise. On Android, the raw ONNX model is bundled directly into a native app and run with ONNX Runtime for Android's default CPU execution provider, with no NNAPI or GPU acceleration enabled. On iOS, the ONNX model is instead converted ahead of time to a native Core ML package and run through Apple's Core ML runtime. On each platform, one representative image per AvocadoLeaf class (6 images) was displayed full-screen on a second device and presented to that platform's own camera, with prediction confidence, per-frame latency, and FPS logged live. Figure~\ref{fig:imac_deployment} shows one such capture on the Mac platform, correctly identifying a \textit{moctrang} (powdery mildew) leaf at 59.0\% confidence, 14.2 FPS, 36.51\,ms latency. These numbers measure the exported classification model alone. The background-removal preprocessing step (Section~\ref{sec:dataset_construction}) is not included in this on-device pipeline, so true end-to-end latency including preprocessing would be higher, particularly on mobile hardware.

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{img/experiments/realtime_demo/imac_deployment.jpg}
\caption{BoVien running live on a Mac (iMac), correctly classifying a phone-displayed \textit{moctrang} (powdery mildew) leaf at 59.0\% confidence, 36.51\,ms latency (14.2 FPS).}
\label{fig:imac_deployment}
\end{figure}

Table~\ref{tab:realtime_demo} reports the mean confidence, FPS, and latency per platform, averaged over the six per-class captures. The full per-class breakdown is in \ref{app:realtime_demo}. Windows is fastest (33.6 FPS, 15.0\,ms), a CPU-only ONNX Runtime session on laptop compute with no mobile-hardware constraint. Among the two genuinely on-device mobile deployments, iOS (native Core ML runtime, with access to Apple's Neural Engine and GPU) is substantially faster than Android (ONNX Runtime's CPU execution provider only, no NNAPI or GPU acceleration enabled) at 17.7 FPS versus 2.7 FPS, a more than six-fold gap that reflects the accelerator each deployment path has access to rather than a difference in the underlying model. The Mac falls between the two laptop and two mobile results (14.5 FPS). Mean confidence also varies by platform (61.0\% to 84.6\%), which is expected. Each platform's camera, lighting, and capture angle differ, and this reflects real deployment variability rather than a change in the underlying model's accuracy, which is fixed by the AvocadoLeaf test set result in Table~\ref{tab:arch_compare}. Even the slowest platform (Android, 2.7 FPS) remains well within the range usable for a farmer pointing a phone at a leaf and waiting a fraction of a second for a diagnosis, though it is far from Windows's real-time responsiveness.

\begin{table}[htbp]
\centering
\caption{Interactive on-device inference  inference of BoVien (ONNX export) across four platforms, one live capture per AvocadoLeaf class (mean over 6 classes). Windows and macOS run the ONNX model on laptop compute via ONNX Runtime (CPU, and CPU/Core ML respectively). Android runs the same ONNX model natively via ONNX Runtime for Android (CPU only). iOS runs a Core ML conversion of the model natively. \emph{Excludes background-removal preprocessing, see text.}}
\label{tab:realtime_demo}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccc}
\toprule
\textbf{Platform} & \textbf{Mean conf.\ (\%)} & \textbf{FPS} & \textbf{Mean latency (ms)} \\
\midrule
Windows & 61.0 & 33.6 & 15.0 \\
macOS   & 79.9 & 14.5 & 30.6 \\
iOS     & 84.6 & 17.7 & 51.6 \\
Android & 76.2 & 2.7  & 283.6 \\
\bottomrule
\end{tabular}
\end{table}
