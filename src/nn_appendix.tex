...
\]
where \(k\) is the number of clusters. Lower DBI values imply better separation. The index has no fixed upper bound and is best interpreted relative to other clustering configurations on the same data rather than against a universal cut-off.
...
\section{Real-time On-device Deployment, Per-class Results}
\label{app:realtime_demo}

This appendix reports the full per-class breakdown behind the platform-level means in Table~\ref{tab:realtime_demo} (Section~\ref{subsec:realtime_demo}). For each of the four platforms, one representative image per AvocadoLeaf class was displayed full-screen on a second device and presented to that platform's own camera, with the exported BoVien model, run via ONNX Runtime (Windows, macOS, Android) or a native Core ML conversion (iOS), producing one live prediction logged per class.

\begin{table}[htbp]
\centering
\caption{Per-class confidence, FPS, and latency for the real-time on-device deployment of BoVien, one live capture per class per platform.}
\label{tab:realtime_demo_per_class}
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{llccc}
\toprule
\textbf{Platform} & \textbf{Class} & \textbf{Conf.\ (\%)} & \textbf{FPS} & \textbf{Latency (ms)} \\
\midrule
Windows & \texttt{binhthuong} & 85.6 & 35.2 & 15.51 \\
Windows & \texttt{chaymepla}  & 74.7 & 37.6 & 13.59 \\
Windows & \texttt{domla}      & 35.7 & 28.9 & 19.00 \\
Windows & \texttt{gisat}      & 68.0 & 33.3 & 14.00 \\
Windows & \texttt{moctrang}   & 65.3 & 36.3 & 13.00 \\
Windows & \texttt{repsap}     & 36.7 & 30.3 & 15.01 \\
\midrule
macOS   & \texttt{binhthuong} & 95.8 & 14.3 & 31.60 \\
macOS   & \texttt{chaymepla}  & 81.0 & 14.9 & 31.41 \\
macOS   & \texttt{domla}      & 71.9 & 15.1 & 30.19 \\
macOS   & \texttt{gisat}      & 85.2 & 14.6 & 28.21 \\
macOS   & \texttt{moctrang}   & 70.2 & 13.4 & 30.90 \\
macOS   & \texttt{repsap}     & 75.5 & 14.6 & 31.40 \\
\midrule
iOS     & \texttt{binhthuong} & 98.8 & 18.2 & 42.10 \\
iOS     & \texttt{chaymepla}  & 90.2 & 17.3 & 55.30 \\
iOS     & \texttt{domla}      & 92.1 & 18.2 & 52.60 \\
iOS     & \texttt{gisat}      & 86.1 & 18.5 & 52.10 \\
iOS     & \texttt{moctrang}   & 81.0 & 16.2 & 53.90 \\
iOS     & \texttt{repsap}     & 59.5 & 17.6 & 53.40 \\
\midrule
Android & \texttt{binhthuong} & 89.6 & 2.7  & 243.09 \\
Android & \texttt{chaymepla}  & 80.7 & 2.6  & 274.34 \\
Android & \texttt{domla}      & 93.0 & 2.5  & 350.30 \\
Android & \texttt{gisat}      & 92.9 & 2.7  & 272.27 \\
Android & \texttt{moctrang}   & 55.7 & 2.9  & 270.02 \\
Android & \texttt{repsap}     & 45.0 & 2.6  & 291.69 \\
\bottomrule
\end{tabular}
\end{table}

The per-class confidence figures should not be read as a second accuracy measurement. Each row is a single live capture per class per platform, so unlike Table~\ref{tab:arch_compare}'s 280-image test-set evaluation, a low confidence value here (e.g.\ Android's 45.0\% on \textit{repsap}) reflects one particular camera angle, lighting condition, and phone-screen glare on a single frame, not a per-class accuracy estimate. This appendix logs confidence and latency only, not whether the predicted class matched the displayed one on each of the 24 captures.
