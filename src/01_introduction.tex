...
\begin{enumerate}
    ...
    \item \textbf{Quantitative, farmer-facing explainability.} A full-test-set LIME deletion-metric evaluation confirms that BoVien's predictions are grounded in disease-relevant leaf regions rather than background artefacts: hiding the $\sim$20\% of the image LIME attributes each prediction to drops the model's confidence by a mean of 0.649 (95\% CI [0.618, 0.678], $n=280$). Building on this, we combine the CNN's prediction, LIME's visual attribution, and a small, free, locally-hosted vision-language model to generate grounded, natural-language explanations of individual predictions, while keeping the CNN as the sole decision-maker (Section~\ref{subsec:vlm_explain}).
...
Figure~\ref{fig:pipeline_overview} summarises how these contributions fit together, from field data collection through architecture selection to the explainability, robustness, and edge-deployment evaluation that follows.
...
% ============================================================
% Connector, from Dataset Construction to Systematic Study
% ============================================================
\draw[arr, line width=1.3pt] (dataleft.east) -- (studyright.west);
